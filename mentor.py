...
class Mentor(object):
	...
	def build(self, rgb, num_classes, temp_softmax, train_mode):
		...
		with tf.name_scope('mentor_conv5_2') as scope:
			kernel = tf.Variable(tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32,
													 stddev=1e-2), trainable = self.trainable, name='mentor_weights')
			conv = tf.nn.conv2d(self.conv11_1, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(tf.constant(0.0, shape=[512], dtype=tf.float32),
								 trainable=self.trainable, name='mentor_biases')
			out = tf.nn.bias_add(conv, biases)
		...
		with tf.name_scope('mentor_fc3') as scope:
			...
			self.parameters += [fc3w, fc3b]
                ...
                return self.conv1_1, self.conv1_2, self.conv2_1, self.conv2_2,self.conv3_1, self.conv3_2, self.conv3_3, self.conv4_1, self.conv4_2, self.conv4_3, self.conv5_1, self.conv5_2, self.conv5_3, self.fc3, tf.nn.softmax(self.fc3/temp_softmax)
