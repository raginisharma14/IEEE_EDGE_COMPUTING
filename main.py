	...
	with tf.Graph().as_default():
		...
		summary = tf.summary.merge_all()
		        ...
		        mentor = VGG16()
                    ...
                    num_batches_per_epoch = FLAGS.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN / FLAGS.batch_size
                    decay_steps = int(num_batches_per_epoch * NUM_EPOCHS_PER_DECAY)
                    _,_,_,_,_,_,_,softmax_mentee = student.build(images_placeholder, FLAGS.num_classes, FLAGS.temp_softmax, phase_train)
				...
				feed_dict = fill_feed_dict(data_input_train, images_placeholder,
					...
					print ('Step %d: loss_value = %.2f' % (i, loss_value))
                                if FLAGS.dependent_student and FLAGS.multiple_optimizers:
                                    ...
                                    if i % 10 == 0:
					print ('Step %d: loss_value = %.2f' % (i, loss_value0))
					print ('Step %d: loss_value2 = %.2f' % (i, loss_value2))
					print ('Step %d: loss_value3 = %.2f' % (i, loss_value3))
